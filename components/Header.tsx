...
export const Header: React.FC<HeaderProps> = ({ user, onLogout, onSearch, onMenuToggle }) => {
  ...
  // Real-time push: prepend WebSocket notifications as they arrive
  ...
  return (
    <header className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-6 md:px-10 sticky top-0 z-10 box-border">
      <div className="flex items-center gap-4 flex-1 max-w-xl">
        ...
        <button
          ...
          className="md:hidden p-2 text-slate-400 hover:text-[#003366] hover:bg-slate-50 rounded-xl transition-all"
        >
        ...
        <form onSubmit={handleSearchSubmit} className="flex-1 relative group">
          ...
          <input
            ...
            aria-label="Search plans or properties"
                ...
                {notifications.length === 0 ? (
                  ...
                  notifications.map(n => (
                    <button
                      ...
                      className={`w-full text-left p-4 rounded-2xl transition-colors group mb-1 ${n.is_read ? 'bg-transparent hover:bg-slate-50' : 'bg-blue-50/50 hover:bg-blue-50'}`}
                    >
